...
FullyHomomorphic::FullyHomomorphic (SecuritySettings *security_settings) : sec(security_settings) 
{
  ...
  srand(init_seed);
  printf("seed: %u\n", init_seed);
  // capable of generating cryptographically sound bytes with input entropy
  rng.Put((byte*) &init_seed, sizeof(unsigned int));
  // rand state is a cryptographic state variable of an algo
...
void FullyHomomorphic::key_gen (PrivateKey &sk, PublicKey &pk) 
{
  ...
  mpz_init(ssk);
  SomewhatPublicKey spk = new __mpz_struct* [sec->public_key_old_key_length]; //1024 + 4 + 1
  SomewhatPublicKey spk2 = new __mpz_struct* [sec->gamma+1]; // 1024 + 1
...
void FullyHomomorphic::create_somewhat_private_key(mpz_t result) 
{
  ...
  // reserve as many as eta bits
  mpz_setbit(temp, sec->eta-2); 
  // now selected 2^(eta-2)
  mpz_add_ui(temp, temp, 1);
  // first part
  mpz_urandomm(result, rand_state, temp); 
  // select range (0 - temp-1) ie 2^(eta)
...
void FullyHomomorphic::choose_random_d(mpz_t result, SomewhatPrivateKey p) 
{
  ...
  mpz_urandomm(temp, rand_state, range); // pick a q that is obtained when dividing a num in the range by p
  mpz_clear(range);
  ...
  mpz_urandomm(temp, rand_state, range); // pick a r ... very similar
  mpz_clear(range);
...
void FullyHomomorphic::print_cipher_bit(const CipherBit &c) 
{
  ...
  printf("} } ");
  unsigned long int i = mpz_sizeinbase(c.old_ciphertext, 2);
  printf("size: %lu\n", i);
}
...
// for 1 to 1029
  for (unsigned int i = 1; i < sec->public_key_old_key_length; i++) 
  {
	...
	{
	  mpz_add(sum, sum, pk.old_key[i]); // really?
	}
	randomness_counter++;
  }
  // now sum contains the value of the sum of the pub key taken from random places
  ...
  mpz_sub_ui(upper_bound, upper_bound, 2);// upper_bound now has value 2^rho
  ...
  mpz_sub_ui(offset, offset, 1);// -2^rho

  mpz_sub(r, r, offset);
  mpz_clear(offset);
  // r should now be in the range (-2^rho', 2^rho')
  ...
  // result if m + 2r + 2 sigma thing in page 6
